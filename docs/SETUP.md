...
SQLはmigration番号順に適用します。現在は`202609130001_foundation.sql`、`202609130002_admin_management.sql`、`202609140003_password_minimum_length.sql`、`202609140004_private_game_submissions.sql`です。既存プロジェクトで同名スキーマ/関数がある場合は先に競合を調べます。移行済みSQLの書き換えではなく、新しいmigrationで変更してください。
